    ...
    route("/posts") {
        authenticate(FIREBASE_AUTH) {
            get {
                ...
                val authorId = call.queryParameters["author_id"]
                    ?.toId(::UserId)
            ...
            post {
                ...
                val recipeId = body.recipeId?.let {
...
private fun VoteInfo.toDto() = PostVoteResponseDto(
    rating = rating,
...
private fun UserPreview.toDto() = PostAuthorDto(
...
private fun Post.toDto() = PostResponseDto(
    ...
    recipe = recipeId?.let {
        PostRecipeRefDto(
            id = it.toString(),
            status = PostRecipeStatusDto.AVAILABLE
        )
    },
    rating = rating,
    userVote = userVote.toDto(),
...
private fun GetPostError.toHttp() = when (this) {
    GetPostError.NotFound -> HttpStatusCode.NotFound to ApiError(
        ApiErrorCode.POST_NOT_FOUND,
        "Post not found",
    )
}

private fun CreatePostError.toHttp() = when (this) {
    is CreatePostError.ValidationError -> error.toHttp()
    CreatePostError.RecipeNotFound -> HttpStatusCode.NotFound to ApiError(
        ApiErrorCode.RECIPE_NOT_FOUND,
        "The attached recipe not found",
    )

    is CreatePostError.ImagesNotUploaded -> HttpStatusCode.UnprocessableEntity to ApiError(
        ApiErrorCode.INVALID_INPUT,
        "The following images were not uploaded: ${this.ids.joinToString(", ")}",
    )
}

private fun UpdatePostError.toHttp() = when (this) {
    UpdatePostError.Forbidden -> HttpStatusCode.Forbidden to ApiError(
        ApiErrorCode.FORBIDDEN, "Forbidden"
    )

    UpdatePostError.NotFound -> HttpStatusCode.NotFound to ApiError(
        ApiErrorCode.POST_NOT_FOUND,
        "Post not found",
    )
...
private fun VotePostError.toHttp() = when (this) {
    VotePostError.PostNotFound -> HttpStatusCode.NotFound to ApiError(
        ApiErrorCode.POST_NOT_FOUND,
        "Post not found",
    )
}

private fun CreatePostCommentError.toHttp() = when (this) {
    CreatePostCommentError.PostNotFound ->
        HttpStatusCode.NotFound to ApiError(
            ApiErrorCode.POST_NOT_FOUND,
            "Post not found",
        )
...
fun PostContentValidationError.toHttp() = when(this) {
    PostContentValidationError.TitleBlank -> HttpStatusCode.UnprocessableEntity to ApiError(
        ApiErrorCode.INVALID_INPUT,
        "The title cannot be empty",
    )
    PostContentValidationError.NoContent -> HttpStatusCode.UnprocessableEntity to ApiError(
        ApiErrorCode.INVALID_INPUT,
