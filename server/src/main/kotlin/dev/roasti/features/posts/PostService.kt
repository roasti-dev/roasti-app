...
class PostServiceImpl(
    ...
    override suspend fun create(
        userId: UserId,
        input: PostInput
    ): Either<CreatePostError, Post> = either {
        input.validate().mapLeft { CreatePostError.ValidationError(it) }.bind()

        input.recipeId?.let { recipeId ->
            recipeService.getById(recipeId, userId)
                .getOrElse { raise(CreatePostError.RecipeNotFound) }
        }
...
//        if (missing.isNotEmpty()) raise(CreatePostError.ImagesNotUploaded(missing))
    ...
    private fun PostInput.validate(): Either<PostContentValidationError, Unit> = either {
        title?.let {
            ensure(it.isNotBlank()) { PostContentValidationError.TitleBlank }
        }

        if (text.isNullOrBlank() && images.isEmpty())
            raise(PostContentValidationError.NoContent)
    }
    ...
    override suspend fun createComment(
        ...
        commentService.create(userId, postId.value, CommentTargetType.POST, text, parentId)
            .mapLeft { CreatePostCommentError.CommentError(it) }.bind()
    }
