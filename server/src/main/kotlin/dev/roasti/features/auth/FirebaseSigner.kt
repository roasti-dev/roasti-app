...
class FirebaseSignerImpl(
  ...
  override suspend fun signInWithPassword(email: String, password: String): SignInResult =
      withContext(Dispatchers.IO) {
        ...
        if (response.statusCode() != HttpStatusCode.OK.value) {
          ...
          throw when (err) {
            "INVALID_PASSWORD",
            "EMAIL_NOT_FOUND" -> AuthException.InvalidCredentials
  ...
  override suspend fun refreshToken(refreshToken: String): SignInResult =
      withContext(Dispatchers.IO) {
        ...
        if (response.statusCode() != HttpStatusCode.OK.value) {
          ...
          throw when (err) {
            ...
            "TOKEN_REVOKED" -> AuthException.InvalidRefreshToken
            "USER_DISABLED" -> AuthException.UserDisabled
  ...
  private fun parseError(body: String): String =
      runCatching { json.decodeFromString<FirebaseErrorResponse>(body).error.message }
          .getOrElse { "UNKNOWN" }
}
...
@Serializable private data class FirebaseErrorResponse(val error: FirebaseError)

@Serializable private data class FirebaseError(val message: String)
