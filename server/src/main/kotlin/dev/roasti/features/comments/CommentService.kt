...
class CommentServiceImpl(private val repo: CommentRepository) : CommentService {
    ...
    override suspend fun create(
        ...
        if (normalized.isBlank()) raise(CreateCommentError.InvalidInput("comment text must not be empty"))
        if (normalized.length > TEXT_MAX_LENGTH)
            raise(CreateCommentError.InvalidInput("comment text must be at most $TEXT_MAX_LENGTH characters"))
